...
def generate_safe_yara_index(rules_dir):
    ...
    # Générer l'index avec includes
    with open(index_file, "w", encoding="utf-8") as f:
        f.write("// Index YARA auto-généré - Ne pas modifier\n")
        f.write(f"// Fichiers inclunis: {len(yara_files)}\n")
        f.write(f"// Noms de règles uniques: {len(all_rules)}\n")
        if duplicates:
            f.write(f"// ⚠️  {len(duplicates)} noms dupliquées (renommés automatiquement)\n")
        f.write("\n")
        ...
        for yara_file in yara_files:
            ...
            # Ajouter l'include dans l'index
